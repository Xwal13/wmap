...
// Save results in a CSV-like format for easy grepping and vuln scanning
func saveResults(results []PortResult, host, filename, format string, osDetect *OSDResult) error {
    ...
    case "csv":
        return saveResultsCSV(results, host, filename)
    default:
        return saveResultsHostCSV(results, host, filename)
    }
}
...
// Save results in CSV, each row: host,port,proto,service,status
func saveResultsHostCSV(results []PortResult, host, filename string) error {
    // Append mode, write header if file doesn't exist or is zero size
    writeHeader := false
    ...
    defer f.Close()
    if writeHeader {
        f.WriteString("host,port,proto,service,status\n")
    }
    for _, r := range results {
        line := fmt.Sprintf("%s,%d,%s,%s,open\n", host, r.Port, r.Proto, r.Service)
        f.WriteString(line)
    }
    return nil
}

func saveResultsCSV(results []PortResult, host, filename string) error {
    // Standard CSV output with more columns
    f, err := os.Create(filename)
    if err != nil {
        return err
    }
    ...
    defer writer.Flush()
    header := []string{"Host", "Port", "Proto", "Service", "Version", "Banner", "Vulnerabilities", "CVEs", "ExploitDB"}
    writer.Write(header)
    for _, r := range results {
        vulnStr := "-"
        if len(r.Vulns) > 0 {
